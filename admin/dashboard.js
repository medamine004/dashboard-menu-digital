    ...
    snap.forEach(doc => {
      ...
      if (o.timestamp) {
        ...
        if (d.getTime() === today.getTime() && o.status === "completed") {
          ...
          productsCount[name] = (productsCount[name] || 0) + Number(o.qty || 1);
        }
      }
      ...
      if (logs && logs.children.length < 5) {
        logs.innerHTML += `
          <li class="flex justify-between border-b border-gray-700 pb-2">
            ...
            <span class="text-white font-bold">${amount.toFixed(1)} DT</span>
          </li>
        `;
      }
    });
