            ...
            list.innerHTML += `
                <tr class="hover:bg-gray-700/50 transition">
                    <td class="p-4 flex items-center gap-3">
                        <img src="${p.img || 'https://placehold.co/40'}" class="w-8 h-8 rounded object-cover bg-gray-600"> 
                        <span class="text-white">${p.name}</span>
                    </td>
                    <td class="p-4 text-center font-mono font-bold ${s<10?'text-red-500':'text-white'}">${s}</td>
                    <td class="p-4 text-center">
                        ${s<10 ? '<span class="text-red-400 font-bold text-xs bg-red-900/20 px-2 py-1 rounded">FAIBLE</span>' : '<span class="text-green-400 text-xs bg-green-900/20 px-2 py-1 rounded">OK</span>'}
                    </td>
                </tr>`;
        });
    });
